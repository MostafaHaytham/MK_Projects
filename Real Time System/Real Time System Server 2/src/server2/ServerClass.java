...
public class ServerClass implements Runnable {
    ...
    public void run() {
      try {
          ...
          IMediaReader mediaReader = ToolFactory.makeReader(SplitVideo.inputFilename);
        // stipulate that we want BufferedImages created in BGR 24bit color space
        mediaReader.setBufferedImageTypeToGenerate(BufferedImage.TYPE_3BYTE_BGR);
        
        mediaReader.addListener(new SplitVideo.ImageSnapListener());
         ...
         while(mediaReader.readPacket() == null)
         {
          //int    x=0;
           //    System.out.println("In the loop for the "+x++);
            int size = MultiThreadServer3.connections.size();        // gets size of currently connected clients               
                //Loop to send the message to all clients
                for(int i =0; i<size;i++)
                {   
                   //  System.out.println("inner loop "+i);
                     pstream = new PrintStream(MultiThreadServer3.connections.get(i).getOutputStream()); // conects to each clients outputstream and sends data
                
                     if(!SplitVideo.outputFilename.equals(Prev)){
                     
                     Prev = SplitVideo.outputFilename;
                    // String TimeStamp = Prev.substring(34, 47);
                    String TimeStamp = Long.toString(System.currentTimeMillis());
                     System.out.println(TimeStamp);
                     pstream.println(SplitVideo.outputFilename);
